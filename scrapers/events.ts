...
const MASJID_ALIASES: Record<string, string[]> = {
  'Islamic Center of Rowlett': ['islamic center of rowlett', 'icr', 'rowlett masjid', 'masjid rowlett'],
  ...
  'SMS Masjid of Sachse': ['sms masjid of sachse', 'sms masjid', 'sachse masjid', 'masjid of sachse'],
  'Valley Ranch Islamic Center': ['valley ranch islamic center', 'vric', 'valley ranch masjid'],
  'Qalam Institute': ['qalam institute', 'qalam'],
  'Islamic Assoc. of Collin County': ['islamic association of collin county', 'islamic assoc. of collin county', 'iacc', 'plano masjid'],
...
async function scrapeSource(source: { name: EventSource; url: string }): Promise<RawEvent[]> {
  ...
  $(selectors.join(',')).each((_, element) => {
    ...
    const imageUrl = absoluteUrl(node.find('img').first().attr('src') || node.find('img').first().attr('data-src'), source.url)
...
async function upsertEvents(events: RawEvent[]) {
  ...
  if (!events.length) {
    logger.warn('Events', 'No matching community events found. Existing events were deactivated to avoid stale feed.')
    return
  }
